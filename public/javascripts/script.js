...
const mu = class extends HTMLElement {
	constructor() {
		...
		this.value = this.getAttribute('value') || 10;
		...
		this.appendChild(s);
		i.type = "range";
		i.step = this.getAttribute('step') || 1;
		i.min = this.min;
		i.max = this.max;
		i.value = this.value;
		s.innerText = ((this.type == "ratio" ? "1/" : "") + this.value.toString());
	}
	connectedCallback() {
		this.addEventListener("input", this.onInput)
	}
	onInput() {
		this.querySelector("span").innerText = ((this.type == "ratio" ? "1/" : "") + this.querySelector("input").value.toString());
	}
}
...
const Table = function(a) {
	let root = this.root = a || document.body;
	// this.csz = 200;
	this.gridProps().genTable(root);
	this.TableEvents().initManualMode();
	return (this)
}
let t = Table.prototype;
t.genTable = function(a) {
	if (this.table)
		this.table.remove(), this.table = void 0;
	this.table = Me("table");
...
t.initManualMode = function() {
	...
	this.table.addEventListener('mouseleave', this.mouseleave.bind(this));
	return (this);
};
...
const Menu = function(a) {
	...
	this.psi = document.querySelector("#panel-slider-lifespeed input");
	this.pst = document.querySelector("#panel-slider-gridres input");
	this.table = a;
	this.structLib;
	this
		.getStructLib()
...
m.initControls = function() {
	...
	t.pst.onchange = t.pst.oninput = () => {
		t.table.csz = parseInt(t.pst.value);
		t.table.gridProps().genTable(t.table.root);
		t.table.initManualMode();
	}
